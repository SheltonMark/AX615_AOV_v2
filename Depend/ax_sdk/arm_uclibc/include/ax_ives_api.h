...
typedef enum axMD_ALG_MODE_E {
    AX_MD_MODE_REF = 0x0,
    AX_MD_MODE_MFNET = 0x1,
    AX_MD_MODE_BUTT
} AX_MD_ALG_MODE_E;

typedef enum axMD_OBJECT_SIZE_E {
    AX_MD_OBJECT_LARGE = 0,  /* 1/4 image height */
    AX_MD_OBJECT_MEDIUM,     /* 1/8 image height */
    AX_MD_OBJECT_SMALL,      /* 1/16 image height */
    AX_MD_OBJECT_BUTT
} AX_MD_OBJECT_SIZE_E;
...
typedef struct axMD_CHN_ATTR_T {
    ...
    union {
        struct {
            ...
            AX_U8 u8ThrY;
        } stRefParam; /* traditional ref mode result */

        struct {
            AX_CHAR *pStrModelDeploymentPath;
            AX_MD_OBJECT_SIZE_E enObjSize;
            AX_F32 f32Threshold;
            AX_U32 u32FrameInterval;
        } stMfnetParam; /* mfnet mode result */
    };
} AX_MD_CHN_ATTR_T;
...
typedef struct axMD_RESULT_T {
    union {
        struct {
            AX_MD_MB_THR_T      stMbThr;
            AX_MD_MB_SAD_T      stMbSad;
        } stRefResult; /* traditional ref mode result */

        struct {
            AX_U8              *pMotionFlags;      /* output with motion flag data, 0: no motion, 1: motion */
            AX_U32              u32SizeInBytes;    /* size of motion flag buffer in bytes */
            AX_U32              u32Rows;           /* output rows */
            AX_U32              u32Cols;           /* output cols */
        } stMfnetResult; /* mfnet mode result */
    };
} AX_MD_RESULT_T;
...
/// @return 0 if success, otherwise failure
//////////////////////////////////////////////////////////////////////////////////////
AX_S32 AX_IVES_MD_Process(MD_CHN mdChn, AX_IVES_IMAGE_T *pstCur, AX_MD_RESULT_T *pstResult, AX_IVES_CCBLOB_T *pstBlob);
