...
#define AX_VO_WIDTH_MIN                     (16)
#define AX_VO_WIDTH_MAX                     (3840)
#define AX_VO_HEIGHT_MIN                    (4)
#define AX_VO_HEIGHT_MAX                    (8190)
#define AX_VO_WIDTH_ALIGN_VAL               (2)
#define AX_VO_HEIGHT_ALIGN_VAL              (2)
#define AX_VO_STRIDE_ALIGN_VAL              (16)
#define AX_VO_FBC_STRIDE_ALIGN_VAL          (128)
#define AX_VO_DEV_MAX           (1)
...
#define AX_ERR_VO_NOT_SUPPORT                 AX_DEF_ERR(AX_ID_VO, AX_ID_VO_PUB_SMOD, AX_ERR_NOT_SUPPORT)
#define AX_ERR_VO_QUEUE_FULL                  AX_DEF_ERR(AX_ID_VO, AX_ID_VO_PUB_SMOD, AX_ERR_QUEUE_FULL)
#define AX_ERR_VO_TIMED_OUT                   AX_DEF_ERR(AX_ID_VO, AX_ID_VO_PUB_SMOD, AX_ERR_TIMED_OUT)
...
typedef enum {
    ...
    AX_VO_OUTPUT_1920x1200_60,          /* VESA 1920 x 1200 at 60 Hz (non-interlaced) CVT (Reduced Blanking) */
    ...
    AX_VO_OUTPUT_1080x1920_30,          /* For MIPI DSI Tx 1080x1920 at 30 Hz */
...
typedef enum {
    ...
    AX_VO_OUT_FMT_YUV420,
    AX_VO_OUT_FMT_SRGB565,
    AX_VO_OUT_FMT_SRGB565S,
    AX_VO_OUT_FMT_SRGB666,
    AX_VO_OUT_FMT_SRGB888,
    AX_VO_OUT_FMT_SRGB888A,
    AX_VO_OUT_FMT_SRGB888B,
    AX_VO_OUT_FMT_BUTT
} AX_VO_OUT_FMT_E;
typedef enum
{
    AX_VO_CLK_SDR_EDGE_DOWN = 0, /*Single edge mode,The transmitter launches data on the falling edge */
    AX_VO_CLK_SDR_EDGE_UP = 1,   /*Single edge mode,The transmitter launches data on the rising edge */
} AX_VO_CLK_SINGLE_EDGE_E;
...
typedef struct axVO_SYNC_INFO_T {
    ...
    AX_BOOL bDlbClk;    /* ddr mode:double clk */
    AX_U8   u8DelaySel; /* delay value for clock*/
} AX_VO_SYNC_INFO_T;
...
typedef struct axVO_PUB_ATTR_T {
    ...
    AX_U32 u32Flags;
    AX_U32 u32InFifoDepth;
    AX_VO_CLK_SINGLE_EDGE_E enClkEdge;
} AX_VO_PUB_ATTR_T;
...
#define VO_DISPLAY_MODE_FLAG_DBLCLK         (1<<12)
...
typedef struct axVO_ASPECT_REGION_T {
    AX_U16 u16LeftSize;
    AX_U16 u16RightSize;
    AX_U16 u16TopSize;
    AX_U16 u16BottomSize;
} AX_VO_ASPECT_REGION_T;

typedef enum
{
    AX_VO_ASPECT_RATIO_NONE = 0,    /* CLOSE LBA */
    AX_VO_ASPECT_RATIO_AUTO = 1,    /* Fill buffer according to horizontal and vertical alignment specified by eAligns[] params */
    AX_VO_ASPECT_RATIO_MANUAL = 2,
    AX_VO_ASPECT_RATIO_BUTT
} AX_VO_ASPECT_RATIO_E;
...
Example1:
    ...
    VO_ASPECT_RATIO_HORIZONTAL_CENTER
    VO_ASPECT_RATIO_VERTICAL_BOTTOM
    then output image:
...
Example2:
    ...
    VO_ASPECT_RATIO_HORIZONTAL_LEFT
    VO_ASPECT_RATIO_VERTICAL_CENTER
    then output image:
    0                            1920   1980
    |                              |      |
    |      Image Area              | Fill |
    ...
    |                              |      |
*/
    AX_VO_ASPECT_RATIO_HORIZONTAL_CENTER = 0,
    AX_VO_ASPECT_RATIO_HORIZONTAL_LEFT = 1,
    AX_VO_ASPECT_RATIO_HORIZONTAL_RIGHT = 2,
    AX_VO_ASPECT_RATIO_VERTICAL_CENTER = AX_VO_ASPECT_RATIO_HORIZONTAL_CENTER,
    AX_VO_ASPECT_RATIO_VERTICAL_TOP = AX_VO_ASPECT_RATIO_HORIZONTAL_LEFT,
    AX_VO_ASPECT_RATIO_VERTICAL_BOTTOM = AX_VO_ASPECT_RATIO_HORIZONTAL_RIGHT,
} AX_VO_ASPECT_RATIO_ALIGN_E;

typedef struct
{
    AX_VO_ASPECT_RATIO_E eMode;
    AX_U32 nBgColor;
    /* YUV color or RGB color (determined by output format):
    31       23      15      7       0
    |--------|--V/B--|--U/G--|--Y/R--|
    */
    AX_VO_ASPECT_RATIO_ALIGN_E eAligns[2]; /* VO_ASPECT_RATIO_ALIGN: [0]: HORIZONTAL [1]: VERTICAL */
    AX_VO_ASPECT_REGION_T tAspetcRegion;   /* Should be set in ASPECT_RATIO_MANUAL mode */
} AX_VO_ASPECT_RATIO_T;


typedef enum
{
    AX_VO_COAX_10M_BCH = 0,
    AX_VO_COAX_10M_NOBCH,
    AX_VO_COAX_20M_BCH,
    AX_VO_COAX_20M_NOBCH,
    AX_VO_COAX_MODE_BUTT,
} AX_VO_COAX_MODE_E;

typedef struct
{
    AX_BOOL             bCoaxEn;    /*when enabled, need send venc data to vo*/
    AX_VO_COAX_MODE_E   eCoaxMode;  /*venc data will be recoded with the mode firstly, and transmited on hdcvi*/
} AX_VO_COAX_ATTR_T;
...
AX_S32 AX_VO_SendFrame2Disp(VO_DEV VoDev, AX_VIDEO_FRAME_T *pstVoFrame, AX_S32 s32MS);
AX_S32 AX_VO_SetAspectRatio(VO_DEV VoDev, const AX_VO_ASPECT_RATIO_T *pstAspectRatio);
AX_S32 AX_VO_GetAspectRatio(VO_DEV VoDev, AX_VO_ASPECT_RATIO_T *pstAspectRatio);

AX_S32 AX_VO_SetDataPinMap(VO_DEV VoDev, const AX_U8 dataArray[AX_VO_PIN_DATA_BUTT]);
AX_S32 AX_VO_GetDataPinMap(VO_DEV VoDev, AX_U8 dataArray[AX_VO_PIN_DATA_BUTT]);

AX_S32 AX_VO_SetCoaxAttr(VO_DEV VoDev, const AX_VO_COAX_ATTR_T *pstCoaxAttr);
AX_S32 AX_VO_GetCoaxAttr(VO_DEV VoDev, AX_VO_COAX_ATTR_T *pstCoaxAttr);
