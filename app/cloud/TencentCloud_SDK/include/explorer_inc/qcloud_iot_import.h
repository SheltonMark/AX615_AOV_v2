 ...
 * Check platform folder for reference implementations
...
/**
 * @brief Get clock timestamp in millisecond
 *
 * @return  clock timestamp in millisecond
 */
...
int HAL_AT_TCP_Write(uintptr_t fd, const unsigned char *buf, uint32_t len, uint32_t timeout_ms,
                     size_t *written_len);
int HAL_AT_TCP_Read(uintptr_t fd, uint8_t *buf, uint32_t len, uint32_t timeout_ms,
                    uint32_t *read_len);
 ...
 * failure
 */
int HAL_UDP_ReadTimeout(uintptr_t fd, unsigned char *p_data, unsigned int datalen,
...
/**
 * @brief register log save path
 *
 * @param path
 * @return int
 */
int HAL_Reg_Log_Path(const char *path);
#endif
