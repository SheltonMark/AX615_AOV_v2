...
typedef struct {
    ...
    uint32_t mqtt_send_ping_interval_ms;   // MQTT ping packat send interval;

    uint32_t recv_buf_max_size;   // mqtt recv buf max size;
    uint32_t write_buf_max_size;  // mqtt send buf max size;
    ...
    char *host_fmt;  // mqtt host format
} MQTTInitParams;
...
#define DEFAULT_MQTTINIT_PARAMS                                                 \
    {                                                                           \
        "china", NULL, NULL, NULL, NULL, 5000, 240 * 1000, 0, QCLOUD_IOT_MQTT_RX_BUF_LEN, QCLOUD_IOT_MQTT_TX_BUF_LEN, 1, 1, \
        {                                                                       \
            0                                                                   \
        }                                                                       \
    }
...
#define DEFAULT_MQTTINIT_PARAMS                                           \
    {                                                                     \
        "china", NULL, NULL, NULL, 5000, 240 * 1000, 0, QCLOUD_IOT_MQTT_RX_BUF_LEN, QCLOUD_IOT_MQTT_TX_BUF_LEN, 1, 1, \
        {                                                                 \
            0                                                             \
        }                                                                 \
    }
 ...
 * @return QCLOUD_RET_SUCCESS for success, or err code for failure
 */
int IOT_MQTT_Disconnect(void **pClient);
