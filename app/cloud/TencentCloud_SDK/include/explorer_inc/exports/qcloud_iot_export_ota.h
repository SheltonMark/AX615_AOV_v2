 ...
 * @param is_backup: use backup domain name connect
 *
 * @return QCLOUD_RET_SUCCESS when success, or err code for failure
 */
int IOT_OTA_StartDownload(void *handle, uint32_t offset, uint32_t size, bool is_backup);
...
/**
 * @brief Report upgrade process to server
 *
 * @param handle: OTA module handle
 * @param percent:  upgrade process percent
 *
 * @return packet id (>=0) when success, or err code (<0) for failure
 */
int IOT_OTA_ReportUpgradeProcess(void* handle, uint32_t percent);
