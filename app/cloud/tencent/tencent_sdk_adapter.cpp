...
TencentCloudResult TencentSdkAdapter::InitSystem(const TencentCloudInitConfig&) {
#if defined(AOV_ENABLE_TENCENT_SDK)
    // Real iv_sys_init wiring needs product credentials and os_impl_cb from board config.
    // Keep this guarded so host tests never link Tencent static libraries.
    return TencentCloudResult::Error(TencentCloudStatusCode::SdkUnavailable,
                                     -1,
                                     "Tencent iv_sys_init wiring is not configured");
#else
    system_ready_ = true;
