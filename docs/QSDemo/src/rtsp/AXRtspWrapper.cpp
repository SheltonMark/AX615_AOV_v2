...
AX_S32 AX_Rtsp_Init(AX_RTSP_HANDLE *pHandle, const AX_RTSP_ATTR_T *pstAttr, AX_S32 nNum, AX_U16 uBasePort)
{
    ...
    AXRtspServer *rtspServer = new(std::nothrow) AXRtspServer();
    ...
    if (rtspServer) {
        ...
        for (AX_S32 i = 0; i < nNum; i ++) {
            ...
            stAttr[i].stVideoAttr.nWidth = pstAttr[i].stVideoAttr.nWidth;
            stAttr[i].stVideoAttr.nHeight = pstAttr[i].stVideoAttr.nHeight;
