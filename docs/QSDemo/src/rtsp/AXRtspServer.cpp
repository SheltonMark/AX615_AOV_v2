...
void* RtspServerThreadFunc(void *args)
{
    ...
    for (AX_S32 i = 0; i < nMaxNum; i++) {
        ...
        if (stSessionAttr.stVideoAttr.bEnable) {
            ...
            if (stSessionAttr.stVideoAttr.nWidth >= 1920 || stSessionAttr.stVideoAttr.nHeight >= 1080) {
                nMaxFrmSize = VIDEO_DEFAULT_MAX_FRM_SIZE * 4 / 5;
            } else {
                nMaxFrmSize = VIDEO_DEFAULT_MAX_FRM_SIZE * 1 / 2;
            }
