...
const static OSD_FONT_STYLE g_arrOsdStyle[] = {
    ...
    {2048 * 1536, 96, 4, 48, 8},  {2304 * 1296, 96, 4, 48, 8},  {1920 * 1080, 48, 2, 24, 8},  {1280 * 720, 32, 2, 20, 8},  {1024 * 768, 32, 2, 16, 8},
...
AX_VOID *OSD_GenStrBitmapEx(wchar_t *pTextStr, AX_U8 *pBitmapBuffer, AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_S16 sX, AX_S16 sY,
                         ...
                         AX_OSD_ALIGN_TYPE_E enAlign, AX_U8 nPixBit) {
    AX_VOID *pBitmap = OSDFONT_GenBitmapEx(pTextStr, pBitmapBuffer, u32OSDWidth, u32OSDHeight, sX, sY, uFontSize, bIsBrushSide,
                           uFontColorIdx, uBgColorIdx, uSideColorIdx, uInvColorIdx, pCharInvFlag, (OSDFONT_ALIGN_TYPE_E)enAlign, nPixBit);
...
wchar_t* OSD_GetCurrDateStr(wchar_t *szOut, AX_OSD_DATE_FORMAT_E eDateFmt, AX_S32 *nOutCharLen) {
    ...
    AX_U32 nDateLen = MAX_OSD_TIME_CHAR_LEN;

    switch (eDateFmt) {
        case AX_OSD_DATE_FORMAT_YYMMDD1: {
            const wchar_t* format = L"%04d-%02d-%02d";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
            break;
        }
        case AX_OSD_DATE_FORMAT_MMDDYY1: {
            const wchar_t* format = L"%02d-%02d-%04d";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900);
            break;
        }
        case AX_OSD_DATE_FORMAT_DDMMYY1: {
            const wchar_t* format = L"%02d-%02d-%04d";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900);
            break;
        }
        case AX_OSD_DATE_FORMAT_YYMMDD2: {
            const wchar_t* format = L"%04dY%02dM%02dD";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
            break;
        }
        case AX_OSD_DATE_FORMAT_MMDDYY2: {
            const wchar_t* format = L"%02dM%02dD%04dY";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900);
            break;
        }
        case AX_OSD_DATE_FORMAT_DDMMYY2: {
            const wchar_t* format = L"%02dD%02dM%04dY";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900);
            break;
        }
        ...
        case AX_OSD_DATE_FORMAT_YYMMDDWW1: {
            const wchar_t* format = L"%04d-%02d-%02d %ls";
            const wchar_t *wday[] = {L"SUN", L"MON", L"TUE", L"WED", L"THU", L"FRI", L"SAT"};
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, wday[tm.tm_wday]);
            break;
        }
        case AX_OSD_DATE_FORMAT_HHmmSS: {
            const wchar_t* format = L"%02d:%02d:%02d";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_hour, tm.tm_min, tm.tm_sec);
            break;
        }
        case AX_OSD_DATE_FORMAT_YYMMDDHHmmSS: {
            const wchar_t* format = L"%04d-%02d-%02d %02d:%02d:%02d";
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
            break;
        }
        case AX_OSD_DATE_FORMAT_YYMMDDHHmmSSWW: {
            const wchar_t* format = L"%04d-%02d-%02d %02d:%02d:%02d %ls";
            const wchar_t *wday[] = {L"SUN", L"MON", L"TUE", L"WED", L"THU", L"FRI", L"SAT"};
            *nOutCharLen = swprintf(szOut, nDateLen, format, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec, wday[tm.tm_wday]);
            break;
        }
        default: {
            *nOutCharLen = 0;
...
AX_S32 OSD_LoadImage(const AX_CHAR* pszImge, AX_VOID** ppVirAddr, AX_U32 nImgSize) {
    ...
    if (fp) {
        ...
        AX_S32 nFileSize = ftell(fp);

        if (nImgSize != nFileSize || nFileSize <= 0) {
