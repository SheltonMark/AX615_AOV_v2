...
static AX_BOOL g_use_lsb_mode = AX_TRUE;
...
static OSD_ERROR DrawPointEx(AX_U8 *pDataBuffer, AX_S16 x, AX_S16 y, AX_U8 uFontColor, AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_U8 nPixBit);
static OSD_ERROR BrushSide(AX_U16 *pDataBuffer, AX_S16 x, AX_S16 y, AX_U16 uSideColor, AX_S16 uBgColor, AX_U32 u32OSDWidth,
                    AX_U32 u32OSDHeight);
static OSD_ERROR BrushSideEx(AX_U8 *pDataBuffer, AX_S16 x, AX_S16 y, AX_U8 uSideColor, AX_U8 uBgColorIdx,
                                                AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_U8 nPixBit);
...
AX_VOID *OSDFONT_GenBitmapEx(wchar_t *pTextStr, AX_U8 *pBitmapBuffer, AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_S16 sX, AX_S16 sY,
                         AX_U16 uFontSize, AX_BOOL bIsBrushSide, AX_U8 uFontColorIdx, AX_U8 uBgColorIdx,
                         AX_U8 uSideColorIdx, AX_U8 uInvColorIdx, AX_U8 *pCharInvFlag,
                         OSDFONT_ALIGN_TYPE_E enAlign, AX_U8 nPixBit) {
    if (u32OSDWidth > OSD_PIXEL_MAX_WIDTH || u32OSDWidth <= 0) {
    ...
    if (nPixBit % 2 > 0 || nPixBit > 8 || nPixBit == 0) {
        printf("nPixBit %d err !\n", nPixBit);
        return NULL;
    }
    ...
    AX_U8 nMask = (0xFF) >> (8 - nPixBit);
    uFontColorIdx &= nMask;
    uBgColorIdx &= nMask;
    uSideColorIdx &= nMask;
    uInvColorIdx &= nMask;

    // printf("bIsBrushSide %d,uFontColorIdx %d,uBgColorIdx %d,uSideColorIdx %d,nPixBit
    // %d\n",bIsBrushSide,uFontColorIdx,uBgColorIdx,uSideColorIdx,nPixBit);

    AX_U32 buffer_szie = u32OSDHeight * u32OSDWidth * nPixBit / 8;

    AX_U8 bgcolor = 0;
    for (AX_U32 i = 0; i < 8 / nPixBit; i++) {
        bgcolor |= (uBgColorIdx << (i * nPixBit));
    }
    memset(pBitmapBuffer, bgcolor, buffer_szie);

    AX_U16 uLen = wcslen((wchar_t *)pTextStr);
    if (uLen == 0) {
        return NULL;
    }
    ...
    switch (enAlign) {
        ...
        case OSD_ALIGN_TYPE_LEFT_BOTTOM: {
            ...
            for (AX_U16 i = 0; i < uLen; i++) {
                ...
                AX_U8 uColorIdx = (pCharInvFlag && pCharInvFlag[i] == 1) ? uInvColorIdx : uFontColorIdx;
                bitmap = FTGetGlpyhBitMap(pTextStr[i]);
                sBearingY = m_fontSlot->metrics.horiBearingY;
                int y0 = y + uFontSize - (int)(5.0 * uFontSize / 64) - sBearingY / 64;
                for (AX_U16 j = 0; j < bitmap->rows; j++) {
                    for (AX_S16 k = 0; k < bitmap->pitch; k++) {
                        ...
                        for (AX_U16 dot = 0; dot < 8; dot++) {
                            if ((dot8 & 0x80) == 0x80) {
                                if (bIsBrushSide) {
                                    BrushSideEx(pBitmapBuffer, x0 + k * 8 + dot, y0 + j, uSideColorIdx, uBgColorIdx, u32OSDWidth,
                                                u32OSDHeight, nPixBit);
                                }
                                if (OE_WIDTH_BACKWARD_RANGE == DrawPointEx(pBitmapBuffer, x0 + k * 8 + dot, y0 + j, uColorIdx,
                                                                           u32OSDWidth, u32OSDHeight, nPixBit)) {
                                    if (!bExit) {
                                        bExit = AX_TRUE;
                                    }
                                }
                    ...
                    for (AX_S16 k = 0; k < bitmap->pitch; k++) {
                        ...
                        for (AX_U16 dot = 0; dot < 8; dot++) {
                            if ((dot8 & 0x80) == 0x80) {
                                ...
                                if (OE_WIDTH_FORWARD_RANGE == DrawPointEx(pBitmapBuffer, x0 + k * 8 + dot, y0 + j, uColorIdx,
                                                                          u32OSDWidth, u32OSDHeight, nPixBit)) {
                                    if (!bExit) {
                                        bExit = AX_TRUE;
                                    }
                        ...
                        for (AX_U16 dot = 0; dot < 8; dot++) {
                            if ((dot8 & 0x80) == 0x80) {
                                for (AX_U16 hScale = 0; hScale < nScale; hScale++) {
                                    for (AX_U16 wScale = 0; wScale < nScale; wScale++) {
                                        if (bIsBrushSide) {
                                            BrushSideEx(pBitmapBuffer, x0 + (k * 8 + dot) * nScale + wScale, y0 + j * nScale + hScale,
                                                        uSideColorIdx, uBgColorIdx, u32OSDWidth, u32OSDHeight, nPixBit);
                                        }

                                        if (OE_WIDTH_BACKWARD_RANGE == DrawPointEx(pBitmapBuffer, x0 + (k * 8 + dot) * nScale + wScale,
                                                                                   y0 + j * nScale + hScale, uColorIdx, u32OSDWidth,
                                                                                   u32OSDHeight, nPixBit)) {
                                            if (!bExit) {
                                                bExit = AX_TRUE;
                                            }
...
static OSD_ERROR DrawPointEx(AX_U8 *pDataBuffer, AX_S16 x, AX_S16 y, AX_U8 uFontColor, AX_U32 u32OSDWidth,
                                                AX_U32 u32OSDHeight, AX_U8 nPixBit) {
    if (x >= (AX_S16)(u32OSDWidth)) {
        return OE_WIDTH_BACKWARD_RANGE;
    }
    ...
    AX_U8  uPixCntInByte = 8 / nPixBit;
    AX_U32 u32ByteW = u32OSDWidth * nPixBit / 8 ;

    if ((x + y * u32OSDWidth) < u32OSDWidth * u32OSDHeight) {
        AX_U32 pix_buf_pos_x = x * nPixBit / 8;
        AX_U32 pix_dot_pos = g_use_lsb_mode ? (x % (uPixCntInByte)) : (uPixCntInByte - 1 - x % (uPixCntInByte));
        AX_U8 pix_valid = (0xFF) >> (8 - nPixBit);

        // printf("ori %x ,add %x\n", pDataBuffer[pix_buf_pos_x + y* u32OSDWidth],((uFontColor&pix_valid) << (pix_dot_pos * nPixBit)));

        // clear old value
        pDataBuffer[pix_buf_pos_x + y * u32ByteW] &= ~((pix_valid) << (pix_dot_pos * nPixBit));

        // add new value
        pDataBuffer[pix_buf_pos_x + y * u32ByteW] |= ((uFontColor & pix_valid) << (pix_dot_pos * nPixBit));

        // printf("x = %d, pos %d ,y =%d,pos %d,uFontColorIdx = %d,result %x\n",pix_buf_pos_x,pix_dot_pos,y,pix_buf_pos_x + y*
        // u32OSDWidth,uFontColor,pDataBuffer[pix_buf_pos_x + y* u32OSDWidth]);
...
static OSD_ERROR BrushSideEx(AX_U8 *pDataBuffer, AX_S16 x, AX_S16 y, AX_U8 uSideColor, AX_U8 uBgColorIdx,
                                                AX_U32 u32OSDWidth, AX_U32 u32OSDHeight, AX_U8 nPixBit) {
    if ((x >= (AX_S16)(u32OSDWidth)) || ((x + 1) >= (AX_S16)(u32OSDWidth))) {
        return OE_WIDTH_BACKWARD_RANGE;
    }
    ...
    if ((y >= (AX_S16)(u32OSDHeight)) || ((y + 1) >= (AX_S16)(u32OSDHeight))) {
        return OE_HEIGHT_BACKWARD_RANGE;
    }
    ...
    for (AX_S32 i = 0; i < 8; i++) {
        AX_U32 pix_buf_pos_x = pix_posi_arry[i].x * nPixBit / 8;
        AX_U32 pix_dot_pos = g_use_lsb_mode ? (pix_posi_arry[i].x % uPixCntInByte) : (uPixCntInByte - 1 - pix_posi_arry[i].x % uPixCntInByte);
        AX_U8 cur_color = (pDataBuffer[pix_buf_pos_x + pix_posi_arry[i].y * u32ByteW] >> (pix_dot_pos * nPixBit)) & pix_valid;

        // printf("uSideColor %d, uBgColorIdx %d, x %d,y %d,pix_dot_pos %d,cur_color %x,ori %x
        // \n",uSideColor,uBgColorIdx,pix_buf_pos_x,pix_posi_arry[i].y,pix_dot_pos,cur_color,pDataBuffer[pix_buf_pos_x + pix_posi_arry[i].y
        // * u32OSDWidth]);

        if (!(uBgColorIdx ^ cur_color)) {
            pDataBuffer[pix_buf_pos_x + pix_posi_arry[i].y * u32ByteW] |= ((uSideColor & pix_valid) << (pix_dot_pos * nPixBit));
            //  printf("after %x\n",pDataBuffer[pix_buf_pos_x + pix_posi_arry[i].y* u32OSDWidth]);
        }
    }
