...
#define SAMPLE_AI_DEFAULT_BLK_SIZE (1280)
...
#define SAMPLE_ADEC_DEFAULT_BLK_SIZE (2048)
#define SAMPLE_ADEC_STEREO_AAC_BLK_SIZE (12800)
...
} QS_SAMPLE_AUDIO_CAPTURE_PARAM_T;
...
} QS_SAMPLE_AUDIO_PLAY_PARAM_T;

// audio get
typedef struct _QS_SAMPLE_AUDIO_GET_THREAD_PARAM {
    AX_BOOL bExit;
    AX_U32 nCardNum;
    AX_U32 nDeviceNum;
    AX_U32 nChn;
    pthread_t nTid;
    QS_SAMPLE_AUDIO_STREAM_CALLBACK callback;
    AX_VOID *pUserData;
} QS_SAMPLE_AUDIO_GET_THREAD_PARAM_T;

// playe file
#define FILE_NAME_LEN (256)
#define QS_SAMPLE_AUDIO_PLAY_FILE_DEPTH (5)

#define NOTIFY_AFPLAY_FILE(pstPlayFileAttr)              \
    do {                                                 \
        pthread_mutex_lock(&g_sample_audio_play_file_info.mtxPlayFile); \
        axfifo_put(&g_sample_audio_play_file_info.qPlayFile, pstPlayFileAttr, sizeof(QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T)); \
...
#define NOTIFY_AFPLAY_RESULT(pstResult)                \
    do {                                               \
        ...
        axfifo_put(&g_sample_audio_play_file_info.qResult, pstResult, sizeof(QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T)); \
...
} QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T;
...
typedef struct {
    ...
    QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_E ePlayFileStatus;
...
} QS_SAMPLE_AUDIO_PLAY_FILE_INFO_T;

static AX_POOL g_sample_ai_PoolId = AX_INVALID_POOLID;
static QS_SAMPLE_AUDIO_CAPTURE_PARAM_T g_sample_audio_capture_param;

static QS_SAMPLE_AUDIO_GET_THREAD_PARAM_T g_sample_audio_get_thread_param;

static AX_POOL g_sample_adec_PoolId = AX_INVALID_POOLID;
static QS_SAMPLE_AUDIO_PLAY_PARAM_T g_sample_audio_play_param;

static QS_SAMPLE_AUDIO_PLAY_FILE_INFO_T g_sample_audio_play_file_info;
...
static AX_VOID *GetAencStreamThread(AX_VOID *pThreadParam) {
    QS_SAMPLE_AUDIO_GET_THREAD_PARAM_T *t = (QS_SAMPLE_AUDIO_GET_THREAD_PARAM_T *)pThreadParam;
...
static AX_S32 GetAudioStreamThreadStart(const QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T* pstCapEntryParam,
...
static AX_S32 SampleAiInit(const QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T* pstCapEntryParam) {
    AX_S32 s32Ret = -1;
    ...
    if (AX_INVALID_POOLID == g_sample_ai_PoolId) {
        ...
        g_sample_ai_PoolId = QS_COMMON_SYS_CreatePool(nFrameSize, SAMPLE_AI_DEFAULT_BLK_CNT, "AUDIO_CAP");
    }

    if (g_sample_ai_PoolId == AX_INVALID_POOLID) {
        COMM_AUDIO_PRT("QS_COMMON_SYS_CreatePool failed");
        goto EXIT;
    }
...
static AX_S32 SampleAiDeinit(const QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T* pstCapEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SampleAencInit(const QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T* pstCapEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SampleAencDeinit(const QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T* pstCapEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SampleCaptureInit(const QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T* pstCapEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SampleCaptureDeinit(AX_VOID) {
    ...
    QS_SAMPLE_AUDIO_CAPTURE_ENTRY_PARAM_T tEntryParam = {
...
static AX_S32 SampleAoInit(const QS_SAMPLE_AUDIO_PLAY_ENTRY_PARAM_T* pstPlayEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SampleAoDeinit(const QS_SAMPLE_AUDIO_PLAY_ENTRY_PARAM_T* pstPlayEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SampleAdecInit(const QS_SAMPLE_AUDIO_PLAY_ENTRY_PARAM_T* pstPlayEntryParam) {
    AX_S32 s32Ret = -1;
  ...
  /* Step2.0: play stop */
...

    {
        ...
        pstAttr.u32MaxFrameSize = nFrameSize;
        ...
        if (AX_INVALID_POOLID == g_sample_adec_PoolId) {
            g_sample_adec_PoolId = QS_COMMON_SYS_CreatePool(nFrameSize, SAMPLE_ADEC_DEFAULT_BLK_CNT, "AUDIO_PLAY");
        }
...
static AX_S32 SampleAdecDeinit(const QS_SAMPLE_AUDIO_PLAY_ENTRY_PARAM_T* pstPlayEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SamplePlayInit(const QS_SAMPLE_AUDIO_PLAY_ENTRY_PARAM_T* pstPlayEntryParam) {
    AX_S32 s32Ret = -1;
...
static AX_S32 SamplePlayDeinit(AX_VOID) {
    ...
    QS_SAMPLE_AUDIO_PLAY_ENTRY_PARAM_T tEntryParam = {
            .pstDevAttr = pstDevAttr,
...
AX_S32 QS_COMMON_AUDIO_Deinit(AX_VOID) {
    ...
    if (g_sample_audio_capture_param.bEnable) {
        ...
        if (g_sample_ai_PoolId != AX_INVALID_POOLID) {
            QS_COMMON_SYS_DestroyPool(g_sample_ai_PoolId);
    ...
    if (g_sample_audio_play_param.bEnable) {
        ...
        if (g_sample_adec_PoolId != AX_INVALID_POOLID) {
            QS_COMMON_SYS_DestroyPool(g_sample_adec_PoolId);
...
static AX_VOID *ResultCallbackThread(AX_VOID* param) {
    ...
    QS_SetThreadName(szName);
    QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T stPlayResult;
    ...
    while (g_sample_audio_play_file_info.bResultThreadRunning) {
        ...
        if (axfifo_size(&g_sample_audio_play_file_info.qResult) > 0) {
            ...
            if (stPlayResult.callback) {
                QS_SAMPLE_AUDIO_PLAY_FILE_RESULT_T stResult;
...
static AX_VOID ProcessPlayFile(QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T* pstPlayFileAttr) {
    //COMM_AUDIO_PRT("+++");
    ...
    if (!pFile) {
        pstPlayFileAttr->eStatus = QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_ERROR;
    ...
    if (g_sample_audio_play_file_info.bStopPlayFile) {
        pstPlayFileAttr->eStatus = QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_STOP;
    } else if (nLoop >= pstPlayFileAttr->nLoop && pstPlayFileAttr->eStatus != QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_ERROR) {
        pstPlayFileAttr->eStatus = QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_COMPLETE;
    }
...
static AX_VOID* ProcessFileThread(AX_VOID *param) {
    ...
    QS_SetThreadName(szName);
    QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T stPlayFileAttr;
...
static AX_S32 SampleInitAudioPlayFileInfo() {
    ...
    g_sample_audio_play_file_info.ePlayFileStatus = QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_BUTT;
    axfifo_create(&g_sample_audio_play_file_info.qPlayFile, QS_SAMPLE_AUDIO_PLAY_FILE_DEPTH * sizeof(QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T));
    ...
    axfifo_create(&g_sample_audio_play_file_info.qResult, QS_SAMPLE_AUDIO_PLAY_FILE_DEPTH * sizeof(QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T));
...
static AX_S32 SampleAudioPlayStop() {
    ...
    if (g_sample_audio_play_file_info.bInited) {
        ...
        axfifo_destory(&g_sample_audio_play_file_info.qPlayFile);
        axfifo_destory(&g_sample_audio_play_file_info.qResult);
...
AX_S32 QS_COMMON_AUDIO_PlayFile(AX_PAYLOAD_TYPE_E eType,
    ...
    {
        QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T tPlayFileAttr;
        memset(&tPlayFileAttr, 0x00, sizeof(QS_SAMPLE_AUDIO_PLAY_FILE_ATTR_T));
        ...
        tPlayFileAttr.eStatus = QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_BUTT;
        ...
        if (!callback) {
            ...
            if (QS_SAMPLE_AUDIO_PLAY_FILE_STATUS_ERROR != g_sample_audio_play_file_info.ePlayFileStatus) {
                s32Ret = 0;
            }
        } else {
            s32Ret = 0;
...
AX_F32 QS_COMMON_AUDIO_GetCaptureVolume(AX_VOID) {
    ...
    s32Ret = AX_AI_GetVqeVolume(card, device, &fVqeVolume);
...
AX_F32 QS_COMMON_AUDIO_GetPlayVolume(AX_VOID) {
    ...
    s32Ret = AX_AO_GetVqeVolume(card, device, &fVqeVolume);
