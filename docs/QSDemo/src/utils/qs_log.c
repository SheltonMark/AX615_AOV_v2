...
int ax_app_log_level = APP_LOG_LEVEL_INFO;
#ifdef AX_FAST_BOOT
...
void LogInit() {
    AX_CHAR* pEnv = NULL;
    int loglevel = ax_app_log_level;
    pEnv = getenv("QSDEMO_LOG_LEVEL");
    if (pEnv) {
        loglevel = atoi(pEnv);
        if (loglevel >= 0 && loglevel <= APP_LOG_LEVEL_DEBUG) {
            ax_app_log_level = loglevel;
        }
    }
}
...
AX_VOID LogToFile(const AX_CHAR *pFmt, ...) {
    if (APP_LOG_LEVEL_CRIT <= ax_app_log_level) {
        pthread_mutex_lock(&g_mtxLog);
#ifdef AX_FAST_BOOT
        if (!g_pLogFile) {
            g_pLogFile = fopen(g_szLogPath, "w");
