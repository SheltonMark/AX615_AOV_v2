...
AX_S32 AX_RingFifo_Init(AX_RINGFIFO_HANDLE *pHandle, AX_U8* pRingBufExt, AX_S32 nSize, const AX_CHAR* pszName) {
    CAXRingBufferEx *rb = new CAXRingBufferEx(pRingBufExt, nSize, 1, pszName);
    if (rb) {
        if (pHandle) {
...
AX_S32 AX_RingFifo_Put(AX_RINGFIFO_HANDLE pHandle, const AX_VOID *buf, AX_U32 len, const AX_VOID *header, AX_U32 header_len, AX_U64 nPts, AX_BOOL bIFrame) {
    if (!pHandle || !buf || len == 0) {
        return 1;
    }

    CAXRingBufferEx *rb = (CAXRingBufferEx *)pHandle;
    CAXRingElementEx ele((AX_U8*)buf, len, nPts, bIFrame, (AX_U8*)header, header_len);
    if (rb->Put(ele)) {
        return 0;
    }
