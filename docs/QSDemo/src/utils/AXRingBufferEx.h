...
public:
    AX_U32 nTotalSize;  // For internal use only, and aligned to sizeof(CAXRingBufferEx).
    AX_U32 nSize;
    AX_U64 nPts;
    AX_BOOL bIFrame;
    AX_U32 nHeadSize;
    AX_S32 nRefCount;
    AX_U32 nSize2;
    AX_U64 nIndex;     // For internal use only
    AX_U8* pBuf{nullptr};
    AX_U8* pBuf2{nullptr};
...
class CAXRingBufferEx {
public:
    CAXRingBufferEx(AX_VOID* pRingBufExt, AX_U32 nElementBuffSize, AX_U32 nElementCount, const char* pszName = nullptr) {
        m_pRingBufExt = pRingBufExt;
        m_nRingSize = nElementBuffSize * nElementCount;
        m_nRingSize = RING_ALIGN_TO(m_nRingSize, sizeof(CAXRingElementEx)); // Ensure that CAXRingElementEx is not split.
        if (!pRingBufExt) {
            m_pRingBuf = (AX_U8 *)malloc(m_nRingSize);
        } else {
            m_pRingBuf = (AX_U8 *)pRingBufExt;
        }
    ...
    ~CAXRingBufferEx(AX_VOID) {
        ...
        m_nTail = 0;
        m_nPopStart = 0;
        if (!m_pRingBufExt && m_pRingBuf) {
            free(m_pRingBuf);
        }
        m_pRingBufExt = nullptr;
        m_mutex.unlock();
    }
