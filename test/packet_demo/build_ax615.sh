...
echo "[debug] CODE_ROOT=${CODE_ROOT}"
...
# Packet 模块源码
PACKET_SOURCES="
    ${CODE_ROOT}/app/packet/packet_service.cpp
    ${CODE_ROOT}/app/packet/media_packet_router.cpp
    ${CODE_ROOT}/app/packet/packet_service_stub.cpp
"

echo "[build] compiling packet demo with packet module"
echo "  Include paths:"
echo "    -I${LIBMEDIA_INC}"
echo "    -I${SDK_INC}"
echo "    -I${THIRD_PARTY_INC}"
echo "    -I${CODE_ROOT}/app"
echo "    -I${CODE_ROOT}/common"
echo "  Checking app/packet/packet_service.hpp:"
ls -l "${CODE_ROOT}/app/packet/packet_service.hpp" || echo "  ERROR: file not found"
...
"${CXX}" -std=c++17 -O2 -Wall -Wextra \
    ...
    -Wl,--end-group \
    -lpthread -ldl -lm \
    -o "${OUT_DIR}/packet_demo"

echo "[build] output:"
ls -lh "${OUT_DIR}/packet_demo"

# Copy to shared directory for board deployment
SHARED_DIR="/share/mashuntao"
if [ -d "${SHARED_DIR}" ]; then
    cp "${OUT_DIR}/packet_demo" "${SHARED_DIR}/packet_demo"
    chmod +x "${SHARED_DIR}/packet_demo"
    echo "[deploy] copied to ${SHARED_DIR}/packet_demo"
fi
